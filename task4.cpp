...
void print64(void *p){
    ...
    std::cout << "а) " << std::hex << *reinterpret_cast<unsigned long*>(p) << std::endl;
    std::cout << "б) " << std::bitset<32>(*reinterpret_cast<unsigned long*>(p)) << std::endl;
    std::cout << "в) " << std::dec << *reinterpret_cast<unsigned long*>(p) << std::endl;
    std::cout << "г) " << std::hex << *reinterpret_cast<signed long*>(p) << std::endl;
    std::cout << "д) " << std::bitset<32>(*reinterpret_cast<signed long*>(p)) << std::endl;
    std::cout << "е) " << std::dec << *reinterpret_cast<signed long*>(p) << std::endl;

    std::cout.setf(std::ios::fixed, std::ios::floatfield);  // Устанавливает режим fixed notation (фиксированной нотации)
    std::cout.precision(2);  // Устанавливает точность после запятой

    std::cout << "ж) " << std::fixed << *reinterpret_cast<float*>(p) << std::endl;
    std::cout << "з) " << std::scientific << *reinterpret_cast<signed long*>(p) << std::endl; // scientific - научная нотация (с экспонентой)
    std::cout << std::endl;
}
...
int main(){
    ...
    val64 = 0;
    print64(reinterpret_cast<unsigned long *>(&val64));

    val64 = ULONG_MAX;
    print64(reinterpret_cast<unsigned long *>(&val64));

    val64 = LONG_MIN;
    print64(reinterpret_cast<long *>(&val64));

    val64 = LONG_MAX;
    print64(reinterpret_cast<long *>(&val64));

    unsigned long x_64 = 0x8A8B8C8D;
    long y_64 = 6;
    long z_64 = -3;
